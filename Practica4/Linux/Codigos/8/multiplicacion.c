...
{
	int i, n;
	double **matriz1, **matriz2, **multiplicacion;
	time_t t;
	...
	multiplicacion = (double**)calloc(n,sizeof(double*));
	for (i = 0; i < n; i++)
		multiplicacion[i] = (double*)calloc(n,sizeof(double));

	// CREAR DIRECTORIO
	char* path = (char*)calloc(2000,sizeof(char));
	path = "/home/enrike/Escritorio/P4-SO/8/Resultados/multiplicacion.txt";
	...
	multiplicar(matriz1, matriz2, multiplicacion, n); 
	crearArchivo(multiplicacion, n, path);
	
	printf("ARCHIVO MULTIPLICACION ESCRITO\n");
	printf("%s", path);
	printf("\n");
