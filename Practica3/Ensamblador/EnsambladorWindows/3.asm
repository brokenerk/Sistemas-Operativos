segment .data
	cadImprimir	db'Ensamblando en Windows',0xA	;Segundo argumento para la llamada al sistema_WriteConsoleA()
...
_main:	push	dword -11					;Argumento pasado por la pila y usado en _GetStdHandle() para la salida estándar
		...
		mov		[handleConsola],eax			;Devolucion del manejador de consola para escritura en el registro eaz

		xor	 eax,eax						;Limpieza del registro eax(eax=10)
		mov	 eax,23d						;eax=23 caracteres de longitud de la cadena a imprimir
		mov	 [longitudCadena],eax			;Se guarda la longitud en mememoria
		xor	 eax,eax						;Limpieza del registro eax(eax=0)
		mov	 eax,0d							;eax=10 valor del ultimo argumento de _WriteConsoleA()
		...
		xor	 eax,eax						;Limpieza del registro eax(eax=10)
		mov	 eax,0d							;eax=10 valor del argumento de _ExitProcess()
